...
app.post('/api/prompt', async (req, res) => {
  ...
  const fullPrompt = prompt || `Lag en fantasigåte som passer for ${character} ved ${location}. Ikke avslør svaret.`

  try {
    const chatCompletion = await openai.chat.completions.create({
      model: 'gpt-4o', // eller 'gpt-3.5-turbo' hvis du ikke har GPT-4
      messages: [
